...
function prepareBox(account){
    ...
    imap.on('mail',async function () {
                    ...
                    imap.seq.search(['UNSEEN'], async (err, results) => {
                            ...
                            //cambia el estado de los mensajes a leidos. Result tiene un array con las posición de los mensajes
                            const fetch = imap.seq.fetch(results, {
                                bodies: '',
                                struct: true,
                                markSeen: true
                            });
                            //Función que recorre y procesa los mensajes
    ...
    imap.once('error', function (err) {
        console.error('❌ ERROR IMAP PROYECTO:', err && err.message ? err.message : err);
    });
    ...
    try { 
        ...
        for(const account of accounts) {
            ...
            const imapAccount = {
                ...
                port: account.Port,
                tls: account.TLS,
                // user: 'suppliers_sp@aciertapps.com',
                // password: '-Acierta2026*',
                // host: 'imap.ionos.es',
                // port: 993,
                // tls: true,
                ...
                connTimeout: 10000,
                authTimeout: 10000,
                // debug: (msg) => console.log(`[IMAP DEBUG][PROYECTO] ${msg.trim()}`)
